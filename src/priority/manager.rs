...
impl DevicePriorityManager {
    ...
    pub fn find_best_output_device(&self, available_devices: &[AudioDevice]) -> Option<AudioDevice> {
    ...
    fn find_best_device(&self, available_devices: &[AudioDevice], priorities: &[DeviceRule], device_type: &str) -> Option<AudioDevice> {
        ...
        for device in available_devices {
            for rule in priorities {
                if rule.matches(&device.name) && rule.weight > best_weight {
                    ...
                    debug!("Found {} device match: {} (weight: {})", device_type, device.name, rule.weight);
                }
            }
        }
        
        if let Some(ref device) = best_device {
            info!("Best {} device: {} (weight: {})", device_type, device.name, best_weight);
        } else {
